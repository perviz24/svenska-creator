...
def build_enhanced_instructions(params: Dict[str, Any]) -> str:
    ...
    parts.append("SPACING & LAYOUT: Generous margins minimum 8-10% on all sides. Maintain consistent padding between elements (24-32px). Group related items visually with proximity. Use grid alignment - no arbitrary positioning. Create clear visual paths for eye movement (Z-pattern for text-heavy, F-pattern for scan-friendly). Balance text and white space at 50-50 or 40-60 ratio.")
...
async def generate_presenton_presentation(request: PresentonRequest) -> Dict[str, Any]:
    ...
    content_text = content_for_analysis[:10000]  # Limit to 10k chars

...
function generatePPTX(slides: Slide[], courseTitle: string, moduleTitle: string): string {
...
function generatePDFHtml(slides: Slide[], courseTitle: string, moduleTitle: string): string {
  const slidePages = slides.map((slide, index) => `
    <div class="slide-page" style="page-break-after: always; padding: 40px; min-height: 100vh; box-sizing: border-box;">
      <div style="border: 1px solid #e0e0e0; border-radius: 8px; padding: 32px; height: calc(100vh - 80px); display: flex; flex-direction: column; ${slide.backgroundColor ? `background-color: ${slide.backgroundColor};` : 'background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);'}">
...
Deno.serve(async (req) => {
  ...
  try {
    const { slides, courseTitle, moduleTitle, format }: ExportRequest = await req.json();

    console.log(`Exporting ${slides.length} slides as ${format} for "${moduleTitle}"`);
    ...
    if (format === 'pptx') {
      ...
      content = generatePPTX(slides, courseTitle, moduleTitle);
      contentType = 'application/xml';
      filename = `${moduleTitle.replace(/[^a-zA-Z0-9]/g, '_')}_presentation.xml`;
    } else if (format === 'pdf') {
      // Generate HTML that can be printed/saved as PDF
      content = generatePDFHtml(slides, courseTitle, moduleTitle);
      contentType = 'text/html';
      filename = `${moduleTitle.replace(/[^a-zA-Z0-9]/g, '_')}_presentation.html`;
    } else {
